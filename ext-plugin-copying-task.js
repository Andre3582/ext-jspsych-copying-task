 ...
 var jsPsychCopyingTask = (function (jspsych) {
    ...
    const info = {
      ...
      parameters: {
        ...
        item_file_type:{
          ...
          default: 'img',
          description: "What type of file the item is. Can be 'img' (i.e. image files like .png, .jpg, .gif, etc.) or 'svg_path') "
        },
        item_scale: {
        ...
        mid_border_specs: {
          ...
          description: 'Set specifications of the border dividing the model from the workspace & resources. Width and left are proportions of the canvas width.'
        },
        canvas_aspect_ratio: {
          type: jspsych.ParameterType.INT,
    ...
    class CopyingTaskPlugin {
      ...
      trial(display_element, trial) {
        ...
        // create the html 
        var html = '<div id="jspsych-canvas-copying-task">' 
        ...
        // set the top and left positions of grids on the canvas
            // model grid
        var mdl_grid_top  = canvas_height * trial.grid_offset['model_y']   - (n_model_rows * grid_box_size) * 0.5; // vertically centered
        var mdl_grid_left = mid_border_left * trial.grid_offset['model_x'] - (n_model_cols * grid_box_size) * 0.5; // horizontally centered between the mid left border & left canvas edge

            // resource grid
        var rsc_grid_top  = canvas_height * trial.grid_offset['resource_y']  - (n_resource_rows * grid_box_size) * 0.5; // vertically centered
        ...
        var wsp_grid_left = mid_border_right + (rsc_grid_left - mid_border_right) * 0.5  - (n_model_cols * grid_box_size) * 0.5;; // horizontally centered between the mid right border & left resource grid edge
                  ...
                  if (wsp_box.content == null){ // if box is empty
                    ...
                    if ( ( item.itemType == wsp_box.correct_content ) || (trial.allow_incorr_placement) ){ // if item in the corect box OR if we allow incorrect placement
                      ...
                      if ( item.itemType == wsp_box.correct_content ) {
                        ...
                        if ( trial.lock_correct_placement) { // ensure a corectly placed item cannot be moved from it's correct location
                      ...
                      } else { // if incorrect but allowed to place incorrect
                        var event = {
